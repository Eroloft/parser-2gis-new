...
- GUI переведён на `FreeSimpleGUI` вместо снятого с PyPI `PySimpleGUI==4.59.0`.
...
- Фильтры результатов (для всех форматов): без франшиз (дедуп по организации),
  ...
  Доступны в CLI (`--filters.*`), десктоп-GUI и веб-интерфейсе.
...
- Веб-интерфейс (`parser-2gis --web`) — локальный дашборд: направления, фильтры,
  запуск/стоп, живой прогресс, карточки результатов и экспорт в любой формат.
